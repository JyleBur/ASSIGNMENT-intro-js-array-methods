...
const buttonFilter = (event) => {
  ...
  if (event.target.id.includes('books')) {
    const books = referenceList.filter(taco => taco.type.toLocaleLowerCase() === "book");
    renderCards(books);
  }
  if (event.target.id.includes('clearFilter')) {
    const clear = referenceList.filter.clear;
    renderCards(referenceList);
  }
...
const cartTotal = () => {
  ...
  const total = cart.reduce((value1, value2) => value1 +
    value2.price, 0);
